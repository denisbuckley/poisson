...
# - Data Output: The output for the single plot mode is simplified to only
#   show the distance from the origin and the relative bearing.
...
def simulate_dynamic_glide_path_and_draw(
        ...
        end_point=(40000, 40000), fig_width=12, fig_height=12
):
    ...
    print(f"Starting simulation from (0,0) to end point {end_point}")
    print(f"Initial Bearing from origin to end point: {bearing_to_end:.2f}°")
    ...
    while math.hypot(end_point[0] - current_pos[0], end_point[1] - current_pos[1]) > EPSILON:
        ...
        if nearest_thermal:
            ...
            relative_bearing = thermal_bearing - bearing_to_end_degrees
            ...
            print(
                f"\nIntercept {step}: Found thermal at Distance: {thermal_dist:.2f} m, Relative Bearing: {relative_bearing:.2f}°")

        else:
            path_segments.append((path_start, end_point))
            current_pos = end_point
            print(f"\nStep {step}: No thermal found. Gliding straight from {path_start} to end point {end_point}.")
